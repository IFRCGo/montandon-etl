...
def ext_and_transform_idu_historical_data():
    ...
    extraction_obj = IDUExtraction.init_extraction(
        metadata=IDUExtractionMetadata(url=url, type=IDUExtractionMetadataType.QUERY)
    )
...
def ext_and_transform_idu_latest_data():
    ...
    chain(IDUExtraction.task.s(extraction_obj.id), IDUTransformHandler.task.s(extraction_obj.id)).apply_async()
